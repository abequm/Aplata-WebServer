...
namespace DataApiService
{
    ...
    public interface IDataManager
    {
        ...
        Task<T> PostItem<T>(string pointName, string postParams = null);
    ...
    public class DataManager : IDataManager
    {
        ...
        public async Task<T> PostItem<T>(String pointName, string postParams = null)
        {
            try
            {
                //Адрес сервиса с токеном
                ...
                var responseData = await _client.UploadDataTaskAsync(url, "POST", System.Text.Encoding.UTF8.GetBytes(postParams));
                var jsonStr = System.Text.Encoding.UTF8.GetString(responseData);
                var result = JsonSerializer.Deserialize<T>(jsonStr);
                return result;
            }
